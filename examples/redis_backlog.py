"""
Advanced example showing how to use celery_lru_priority with Redis storage.
...
def simulate_mixed_workload(client_count=3, tasks_per_client=5):
    ...
    for i in range(tasks_per_client):
        ...
        for client_id in range(client_count):
            ...
            process_data.lru_delay(f"client_{client_id}", f"Dataset {i} from client {client_id}")
            
            # Every other iteration, also submit an analysis task
            if i % 2 == 0:
                analyze_data.lru_delay(f"client_{client_id}", f"Results {i} from client {client_id}")
