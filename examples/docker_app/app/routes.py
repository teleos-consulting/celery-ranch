...
from .tasks import process_client_data, generate_random_jobs
import random
import datetime
...
def index():
    ...
    jobs = db.query(DataProcessingJob).order_by(DataProcessingJob.created_at.desc()).limit(10).all()
...
def list_jobs():
    db = get_db()
    jobs = db.query(DataProcessingJob).order_by(DataProcessingJob.created_at.desc()).all()
    return jsonify([{
        'id': job.id,
...
def api_generate_jobs():
    ...
    return jsonify({
        'task_id': task.id,
        'status': 'Jobs generation started',
        'num_jobs': num_jobs
    })