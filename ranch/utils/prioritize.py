...
def create_redis_client(app=None):
    ...
    try:
        ...
        is_redis = (
            broker_url.startswith("redis://") or 
        ...
        redis_socket_connect_timeout = ranch_config.get("redis_socket_connect_timeout", 3)
    ...
    with _init_lock:
        ...
        try:
            ...
            if redis_client:
                ...
                redis_serializer = ranch_config.get("redis_serializer", _DEFAULT_CONFIG["redis_serializer"])
                redis_max_retries = ranch_config.get("redis_max_retries", _DEFAULT_CONFIG["redis_max_retries"])
            ...
            # Fall back to in-memory storage
            _storage = InMemoryStorage()
            logger.warning(f"Error initializing Redis storage, falling back to in-memory: {e}")
    ...
    try:
        ...
        if _task_backlog and task_id:
            ...
            if task_data:
                ...
                logger.info(f"Requeuing task {task_id} for client {lru_key} due to error")
                # Delay slightly to prevent immediate retries in case of persistent errors
                time.sleep(0.5)
